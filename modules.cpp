...
string module::corp::get(message *inMsg)
{
	...
	if (!corps["users"][to_string(inMsg->user_id)].is_null())
	{
		...
		msg += "Заработок: " + to_string(mCoff * (int)pow(1.5, corps["corps"][name]["lvl"].get<int>()) * corps["corps"][name]["lvl"].get<int>() * corps["corps"][name]["users"].size()) + "$/мин\n";
...
{
	int t = time(NULL) / 60;
	corps["corps"][name]["money"] = corps["corps"][name]["money"].get<int>() + mCoff * (int)pow(1.1, corps["corps"][name]["lvl"].get<int>()) * corps["corps"][name]["users"].size() * (t - corps["corps"][name]["money_time"].get<int>());
